...
inline void mkdir(const std::string& path, std::filesystem::perms mode) {
...
inline dict parse_args(int argc, char* argv[]) {
  dict kwargs;
...
template <typename T>
inline T get_arg(dict& kwargs, const std::string& key, const T& default_value) {
...
template <>
inline std::string get_arg<std::string>(dict& kwargs, const std::string& key,

#!/usr/bin/env python3
"""Cross-architecture vlp1d_1v bandwidth plots.
...
Data comes from ``artifact/vlasov1d_1v/<backend>/<sha7>/`` (one SHA7 per
...
x-axis: problem size 128²..4096² (semilog, log2-spaced positions);
y-axis: modelled bandwidth GB/s (LINEAR).
"""
...
import fig_common
from parse_cross_platform_vlp1d import collect, SIZES  # reuse parser
...
def series(data, model, arch, prec):
    return [d["bw"] if (d := data[model].get(f"{arch}|{prec}|{n}")) else math.nan
            for n in SIZES]
...
def make_plot(data, prec, group_name, archs, out_dir: Path):
    ...
    for arch in archs:
        ...
        for model in ("jax", "kokkos", "pytorch"):
            ...
            if all(math.isnan(v) for v in ys):
                continue
            ax.plot(x, ys, color=MODEL_COLOR[model], linewidth=2.8,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"${n}^2$" for n in SIZES])
    ...
    out = out_dir / f"vlp1d_bandwidth_{prec}_{group_name}.png"
...
def main():
    ...
    data = collect(arch_map=fig_common.arch_map(args),
                   base=args.artifact_root / "vlasov1d_1v")

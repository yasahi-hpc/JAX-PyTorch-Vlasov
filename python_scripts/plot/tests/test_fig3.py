"""Tests for fig3.py (cross-architecture vlp1d_1v bandwidth, JAX/Kokkos/PyTorch).
...
def artifact_data():
    ...
    return parser.collect(arch_map=arch_map, base=fig_common.ARTIFACT_ROOT / BENCH)
...
def test_writes_images_to_imgs_dir(run_figure):
    ...
    _, start = run_figure("fig3.py", *sha_args())
    for name in EXPECTED:
        assert_written(IMG_DIR / name, start)


def test_out_dir_argument(run_figure, tmp_path):
    _, start = run_figure("fig3.py", *sha_args(), out_dir=tmp_path)
    for name in EXPECTED:

HloModule jit_heat_step_conv, entry_computation_layout={(f64[32,32,32]{2,1,0}, f64[])->f64[32,32,32]{2,1,0}}

heat_step_conv.1 {
  Arg_0.1 = f64[32,32,32]{2,1,0} parameter(0)
  Arg_1.1 = f64[] parameter(1)
  mul.2 = f64[32,32,32]{2,1,0} broadcast(Arg_1.1), dimensions={}
  transpose.4 = f64[32,32,32]{1,0,2} transpose(Arg_0.1), dimensions={1,2,0}
  ...
  transpose.6 = f64[32,32,32]{1,2,0} transpose(Arg_0.1), dimensions={0,2,1}
  ...
  reshape.10 = f64[1024,1,32]{2,1,0} reshape(Arg_0.1)
  ...
  ROOT add.5 = f64[32,32,32]{2,1,0} add(Arg_0.1, mul.3)
}

ENTRY main.2 {
  u.1 = f64[32,32,32]{2,1,0} parameter(0)
  coeff.1 = f64[] parameter(1)
  constant.1 = f64[1,1,3]{2,1,0} constant({ { { 1, -2, 1 } } })
  ROOT jit_heat_step_conv_.1 = f64[32,32,32]{2,1,0} call(u.1, coeff.1, constant.1), to_apply=heat_step_conv.1
}

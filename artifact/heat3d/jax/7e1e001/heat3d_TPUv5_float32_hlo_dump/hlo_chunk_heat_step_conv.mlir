...
heat_step_conv.1 {
  ...
  mul.2 = f32[32,32,32]{2,1,0} broadcast(Arg_1.2), dimensions={}
  transpose.4 = f32[32,32,32]{1,0,2} transpose(Arg_0.2), dimensions={1,2,0}
  ...
  transpose.6 = f32[32,32,32]{1,2,0} transpose(Arg_0.2), dimensions={0,2,1}
  ...
  reshape.10 = f32[1024,1,32]{2,1,0} reshape(Arg_0.2)
  ...
  ROOT add.5 = f32[32,32,32]{2,1,0} add(Arg_0.2, mul.3)
}

closed_call.2 {
  Arg_1.3 = f32[32,32,32]{2,1,0} parameter(1)
  constant.7 = f32[] constant(0.0259382222)
  Arg_0.3 = f32[1,1,3]{2,1,0} parameter(0)
  ROOT jit_heat_step_conv_.1 = f32[32,32,32]{2,1,0} call(Arg_1.3, constant.7, Arg_0.3), to_apply=heat_step_conv.1
}
...
region_0.3 {
  ...
  add.7 = s32[] add(get-tuple-element.2, constant.8)
  constant.9 = f32[1,1,3]{2,1,0} constant({ { { 1, -2, 1 } } })
  get-tuple-element.3 = f32[32,32,32]{2,1,0} get-tuple-element(arg_tuple.1), index=1
  closed_call.1 = f32[32,32,32]{2,1,0} call(constant.9, get-tuple-element.3), to_apply=closed_call.2
  ROOT tuple.1 = (s32[], f32[32,32,32]{2,1,0}) tuple(add.7, closed_call.1)
}
...
ENTRY main.5 {
  ...
  while.5 = (s32[], f32[32,32,32]{2,1,0}) while(while.4), condition=region_1.4, body=region_0.3
  ...
  constant.12 = f32[] constant(0.02)
  add.10 = f32[] add(start_state_2_.1, constant.12)
  start_state_3_.1 = s32[] parameter(2)
  constant.13 = s32[] constant(20)
  add.11 = s32[] add(start_state_3_.1, constant.13)
  ROOT tuple.3 = (f32[32]{0}, f32[32]{0}, f32[32]{0}, f32[32,32,32]{2,1,0}, f32[], /*index=5*/s32[]) tuple(constant.14, constant.14, constant.14, while.7, add.10, add.11)
}
